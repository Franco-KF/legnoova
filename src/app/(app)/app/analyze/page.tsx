    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Upload a screenshot of your chart and let the AI read the structure to
          generate risk-aware entry, stop-loss and dynamic take-profit targets.
        </p>
      </div>
        ...
        <div className="space-y-5">
          ...
          <div className="glass-panel p-5">
            ...
            <div className="space-y-4">
              ...
              {/* Symbol hint */}
              <div>
                <label htmlFor="symbol" className="mb-1.5 block text-xs font-medium text-muted-foreground">
          ...
          >
            {loading ? (
              <>
                ...
                Analysing chart with Gemini…
              </>
            ) : (
              <>
                <Sparkles className="h-4 w-4" />
                {canAnalyze ? "Generate Signal" : "Upload a chart to analyze"}
              </>
            )}
          </button>
        </div>
  ...
  return (
    <div className="flex h-full min-h-[420px] flex-col items-center justify-center rounded-2xl border border-white/[0.06] bg-white/[0.01] p-10 text-center">
      ...
      <p className="mt-2 max-w-xs text-sm text-muted-foreground/70">
        Upload a chart screenshot and choose a strategy to see your AI-generated
        signal here — including dynamic take-profit targets (TP1, TP2 and up to TP3).
      </p>
    </div>
  );
}
...
function ResultPanel({ analysis }: { analysis: Analysis }) {
  const neutral = analysis.direction === "neutral" || analysis.takeProfits.length === 0;
  ...
  return (
    <div className="space-y-5 animate-fade-in-up">
      ...
      <div className="glass-panel overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-heading text-xl font-bold">{analysis.pair || analysis.symbol}</h2>
                {analysis.timeframe && (
        ...
        {!neutral && (
          <div className="grid grid-cols-3 gap-3 p-5">
            <Metric label="Entry" value={formatPrice(analysis.entryPrice, analysis.symbol)} accent />
            <Metric label="Stop Loss" value={formatPrice(analysis.stopLoss, analysis.symbol)} danger />
            <Metric
              label="Risk:Reward"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {tp.explanation && (
                      <p className="text-xs text-muted-foreground">{tp.explanation}</p>
                    )}
                  </div>
                </div>
      ...
      {neutral && (
        <div className="glass-panel p-6 text-center">
          ...
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            The AI didn&apos;t find a high-probability trade in this chart. This is a
              ...
              <div className="space-y-1">
                {analysis.keyLevels.resistance.map((r, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg bg-white/[0.02] px-3 py-2">
                    <span className="font-mono text-sm">{formatPrice(r, analysis.symbol)}</span>
                  </div>
                ))}
            ...
            <div>
              ...
              <div className="space-y-1">
                {analysis.keyLevels.support.map((s, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg bg-white/[0.02] px-3 py-2">
                    <span className="font-mono text-sm">{formatPrice(s, analysis.symbol)}</span>
                  </div>
                ))}
      ...
      {/* Strategy assessment */}
      {analysis.strategyAssessments && analysis.strategyAssessments.length > 0 && (
        <div className="glass-panel p-5">
          <h3 className="mb-3 font-heading text-sm font-semibold">Strategy Read</h3>
          <div className="space-y-2">
